...
@section('header_title', 'Tambah Proyek')
...
<div class="max-w-5xl space-y-6">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 shadow-sm">
        <h2 class="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4 mb-6">Form Tambah Proyek Baru</h2>
        ...
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Contoh: Sentiment Analysis Pipeline"
            ...
            <div>
                ...
                <input type="text" name="title_id" id="title_id" value="{{ old('title_id') }}" placeholder="Contoh: Pipeline Analisis Sentimen" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category') }}" required placeholder="Contoh: Data Engineering, Web Dev"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                    @error('category')
                        <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="category_id" id="category_id" value="{{ old('category_id') }}" placeholder="Contoh: Rekayasa Data" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="tags" id="tags" value="{{ old('tags') }}" placeholder="Contoh: Python, PostgreSQL, Airflow, Spark"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
            ...
            <div>
                ...
                <input type="file" name="attachments[]" id="attachments" multiple accept=".pdf,.doc,.docx,.ppt,.pptx,.zip,.rar,.jpg,.jpeg,.png,.webp,.mp4,.webm,.ogg"
                       class="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200">
                <span class="text-[10px] text-slate-400 mt-1 block">Bisa unggah beberapa file sekaligus. Maksimal 10MB per file.</span>
                @error('attachments')
                    <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="url" name="github_url" id="github_url" value="{{ old('github_url') }}" placeholder="https://github.com/username/project"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                    @error('github_url')
                        <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="url" name="demo_url" id="demo_url" value="{{ old('demo_url') }}" placeholder="https://myproject.demo"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                    @error('demo_url')
                        <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6" required placeholder="Jelaskan tentang proyek ini secara detail, tujuan pengerjaan, tantangan, dan solusi yang diimplementasikan..."
            ...
            <div>
                ...
                <textarea name="description_id" id="description_id" rows="6" placeholder="Jelaskan proyek dalam Bahasa Indonesia..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">{{ old('description_id') }}</textarea>
            </div>
