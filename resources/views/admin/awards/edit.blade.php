...
@section('header_title', 'Edit Penghargaan')
...
<div class="max-w-5xl space-y-6">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 shadow-sm">
        <h2 class="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4 mb-6">Form Edit Penghargaan</h2>
        ...
        <form action="{{ route('admin.awards.update', $award->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $award->title) }}" required placeholder="Contoh: Juara 1 Lomba Data Analysis"
            ...
            <div>
                ...
                <input type="text" name="title_id" id="title_id" value="{{ old('title_id', $award->title_id ?: $award->title) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="issuer" id="issuer" value="{{ old('issuer', $award->issuer) }}" placeholder="Contoh: Dicoding, Kampus, Google"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                    @error('issuer')
                        <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="award_date" id="award_date" value="{{ old('award_date', $award->award_date?->format('Y-m-d')) }}"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                    @error('award_date')
                        <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <span class="text-[10px] text-slate-400 mt-1 block">Pilih file baru jika ingin mengganti gambar. Format: JPG, PNG, WEBP. Maks: 4MB.</span>
                    @error('image')
                        <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($award->certificate_path)
                        <div class="mb-3 p-3 rounded-xl border border-amber-100 bg-amber-50/50 flex items-center gap-2">
                            <i class="fa-solid fa-file-lines text-amber-500 text-lg"></i>
                            <a href="{{ asset('storage/' . $award->certificate_path) }}" target="_blank" class="text-xs font-semibold text-amber-700 hover:underline">
                                Lihat file saat ini
                            </a>
                        </div>
                    @endif
                    <input type="file" name="certificate" id="certificate" accept=".pdf,.jpg,.jpeg,.png,.webp"
                           class="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200">
                    <span class="text-[10px] text-slate-400 mt-1 block">Pilih file baru jika ingin mengganti sertifikat. Maks: 5MB.</span>
                    @error('certificate')
                        <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div>
                ...
                <input type="url" name="external_url" id="external_url" value="{{ old('external_url', $award->external_url) }}" placeholder="https://..."
                       class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">
                @error('external_url')
                    <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $award->is_featured) ? 'checked' : '' }}
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" placeholder="Jelaskan pencapaian, konteks lomba, pelatihan, atau sertifikasi..."
                          class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">{{ old('description', $award->description) }}</textarea>
                @error('description')
                    <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            <div>
                ...
                <textarea name="description_id" id="description_id" rows="5" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm">{{ old('description_id', $award->description_id ?: $award->description) }}</textarea>
            </div>
